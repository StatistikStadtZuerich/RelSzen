...
for (iyear in future) {
  ...
  dem <- as_tibble(expand_grid(
    ...
    mutate(pop_end = pmax(0, pop + bir - dea + ims - ems + con),
           age_new = age + 1) %>% 
  ...
  pop_end_year <- sce %>% 
    ...
    mutate(reformed = sce * prop_fit / 100, 
           other = sce - reformed) %>% 
...
dat_yr <- out_pop %>% 
  ...
  sszplot(dat_yr, aes_x = "year", aes_y = "pop", aes_col = "rel",
  i_x = scen_begin,          
...
dat_yr %>% 
  ...
  mutate(prop_ref = round(reformed / (reformed + other) * 100, round_prop)) %>%   
  sszplot(aes_x = "year", aes_y = "prop_ref", i_x = scen_begin,          
...
dat_ya3r %>% 
  pivot_wider(names_from = "rel", values_from = pop) %>% 
  mutate(prop_ref = round(reformed / (reformed + other) * 100, round_prop)) %>%   
  sszplot(aes_x = "year", aes_y = "prop_ref",
  i_x = scen_begin,          
  labs_y = "proportion reformed (in %)", 
  wrap = "age_3", ncol = 4,
...
dat_con_y <- out_dem %>% 
  ...
  sszplot(dat_con_y, aes_x = "year", aes_y = "con",
  i_x = scen_begin,          
...
dat_yr %>% 
  ...
  sszplot(aes_x = "year", aes_y = "con_rate",
  i_x = scen_begin,          
